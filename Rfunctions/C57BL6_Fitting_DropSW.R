...
wparams <- function(outF, d) {
  writeLines(paste(d$Gene,d$params$loggamma,d$params$omega,
...
FitData<-function(swdmr,Gene){
  ...
  # Full model
  full<-FitDropSW(swdmr,Gene)
  
  retv<-c()
  retv<-c(retv,full$optimxres[1,c("loggamma","omega","AmpSin","PhiSin")])
...
outF <- file(OF)
writeLines(paste("Gene","loggamma","omega","AmpSin","PhiSin","RSS","BF","BIC","KendallTau",
                 sep="\t"), outF)
close(outF)
...
fits <- foreach(i = 1:length(Genes),.init=outF,.packages=c("optimx","SWDMr"),.combine="wparams") %dopar% {
  ...
  if (is(tf,"try-error")){fit<-list(loggamma=NA,omega=NA,AmpSin=NA,PhiSin=NA,RSS=NA,BF=NA,BIC=NA,
                                    KendallTau=NA)}
