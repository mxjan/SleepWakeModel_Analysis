...
wparams <- function(outF, d) {
  writeLines(paste(d$Gene,d$params$Wake,d$params$Sleep,d$params$loggamma,d$params$omega,
                   d$params$AmpSin,d$params$PhiSin,d$params$RSS,d$params$BF,d$params$BIC,d$params$KendallTau,
                   sep="\t"), outF)
  return(outF)
}
...
FitData<-function(swdmr,Gene){
  ...
  # Full model
  full<-FitFullModel(swdmr,Gene)

  retv<-c()
  retv<-c(retv,full$optimxres[1,c("Wake","Sleep","loggamma","omega","AmpSin","PhiSin")])
  retv<-c(retv,c(full$stats$stats$RSS, full$stats$stats$BayesFactor, full$stats$stats$BIC, full$stats$stats$KendalTau ))
  names(retv)[seq(7,10)]<-c("RSS","BF","BIC","KendallTau")
  return(retv)
}
...
outF <- file(OF)
writeLines(paste("Gene","Wake","Sleep","loggamma","omega","AmpSin","PhiSin","RSS","BF","BIC","KendallTau",
                 sep="\t"), outF)
close(outF)
...
fits <- foreach(i = 1:length(Genes),.init=outF,.packages=c("optimx","SWDMr"),.combine="wparams") %dopar% {
  ...
  if (is(tf,"try-error")){fit<-list(Wake=NA,Sleep=NA,loggamma=NA,omega=NA,AmpSin=NA,PhiSin=NA,RSS=NA,BF=NA,BIC=NA,
                                    KendallTau=NA)}
